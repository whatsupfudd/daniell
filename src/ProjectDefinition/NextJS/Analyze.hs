...
module ProjectDefinition.NextJS.Analyze where
...
import Options.Runtime (RunOptions (..), TechOptions (NextOptions))
...
analyseStaticProject rtOpts (ProjectDefinition baseDir (Site NextStatic) [] pathFiles) content =
  case rtOpts.techOpts of
    NextOptions nxtOpts ->
      ...
      in
      pure . Right $ WorkPlan {
            engine = NxEngine { nextjsOpts = nxtOpts }
            , context = NxContext {
              mergedConfigs = Mp.empty
              , pathPrefixes = Mp.empty
              , compFileUnits = Mp.empty
              , siteDef = content
              }
            , items = ExecTmplForContentNx "" :| []
        }
    _ -> pure . Left . SimpleMsg $ "[analyseStaticProject] invalid tech options: " <> pack (show rtOpts.techOpts)
  ...
  case rtOpts.techOpts of
    NextOptions nxtOpts ->
      ...
      pure . Right $ WorkPlan {
            ...
            , items = case content.pages of
                [] -> ExecTmplForContentNx "" :| []
                h : rest -> makeWorkItem baseDir h :| map (makeWorkItem baseDir) rest
        }
    _ -> pure . Left . SimpleMsg $ "[analyseWebAppProject] invalid tech options: " <> pack (show rtOpts.techOpts)


makeWorkItem :: FilePath -> Fs.FileWithPath -> NxWorkItem
makeWorkItem rootPath (dirPath, Fs.KnownFile aKind aPath) =
  let
    prefixPath = case dirPath of
      "" -> rootPath </> "src/pages"
      _ -> rootPath </> "src/pages" </> dirPath
    fullPath = prefixPath </> aPath
  in
  case aKind of
    Fs.TsReact -> ExecTmplForContentNx fullPath
    _ -> ExecTmplForRouteNx fullPath
